...
```bash
git clone https://github.com/devSviat/Messaging-OkayCMS.git
```
...
```
{OkayCMS_root}/
├── Okay/Modules/Sviat/Messaging/     ← модуль
└── tests/Modules/Sviat/Messaging/     ← його тести
```
...
```bash
cd {OkayCMS_root}
mkdir -p tests/Modules/Sviat/Messaging
cp -r ~/Messaging-OkayCMS/tests/. tests/Modules/Sviat/Messaging/
```
...
```bash
cd {OkayCMS_root}
vendor/bin/phpunit tests/Modules/Sviat/Messaging
```
...
2. Неймспейс — `Modules\Sviat\Messaging`.
...
use Okay\Modules\Sviat\Messaging\Services\SomeService;
